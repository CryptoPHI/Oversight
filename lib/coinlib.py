...
def is_valid_coin_address(address, network='mainnet'):
    ...
    try:
        ...
        # rescue from exception, not a valid coin address
        return False

    if (address_version != coin_version):
        return False
...
def SHIM_serialise_for_coind(sentinel_hex):
    ...
    # add 'type' attribute
    obj[1]['type'] = COIND_GOVOBJ_TYPES[govtype]

    # superblock => "trigger" in coind
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # coind expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    coind_hex = serialise(obj)
    return coind_hex


# shims 'til we can fix the coind side
def SHIM_deserialise_from_coind(coind_hex):
    from models import COIND_GOVOBJ_TYPES

    # unpack
    obj = deserialise(coind_hex)
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('mue.conf')
